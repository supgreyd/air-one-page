...
(function($){
  ...
  var $topNavElements = $('.header__nav_menu > a');
  $('section[id]').scrollspy(
    {
      outCallback: function ($element) {
        $topNavElements.filter('[href="#' + $element.attr('id') + '"]').removeClass('active_link');
        $element.find('.side').html('');
      },
      inCallback:  function ($element, side) {
        $topNavElements.filter('[href="#' + $element.attr('id') + '"]').addClass('active_link');
